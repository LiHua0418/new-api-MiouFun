...
func inviteUser(inviterId int, quota int) error {
	...
	if quota > 0 {
		...
		updates["aff_history"] = gorm.Expr("aff_history + ?", quota)
	}
...
func (user *User) finishInsert(inviterId int) {
	...
	if inviterId != 0 && operation_setting.IsPaymentComplianceConfirmed() {
		...
		if err := inviteUser(inviterId, common.QuotaForInviter); err != nil {
			common.SysLog(fmt.Sprintf("更新邀请统计失败: inviterId=%d, error=%v", inviterId, err))
