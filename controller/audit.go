...
var auditContentTemplates = map[string]string{
	...
	"user.used_quota_add":      "Increased user historical usage by ${quota} (${from} to ${to})",
	"user.used_quota_subtract": "Decreased user historical usage by ${quota} (${from} to ${to})",
	"user.used_quota_override": "Overrode user historical usage from ${from} to ${to}",
